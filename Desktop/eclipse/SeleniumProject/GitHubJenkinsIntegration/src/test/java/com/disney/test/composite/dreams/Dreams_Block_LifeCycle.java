...
public class Dreams_Block_LifeCycle {
	...
	public Object[][] scenarios() {
		try {
			...
			return dt.getTestScenariosByApp(application, "BlockLifeCycle");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 */
	@Test(dataProvider = "blockScenario", description = "Block LifeCycle", groups = {
			"regression", "dreams", "CreateGroupProfile", "OrgSearch",
			"contactGuestSearch", "GroupInfo", "GroupConfirmations",
			"createBlock", "setBlockDatesAndSelectResort", "BlockInventory",
			"EditBlock", "searchPackageCode", "UpdateAndClearCustomPackage" })
	public void blockLifeCycle(String testScenario, String role,
			String entergroupsProfile, String OrgSearch,
			String contactGuestSearch, String groupInfo,
			String setSpecialConsiderations, String CreateBlock,
			String setBlockDatesAndSelectResort, String setBlockInventory,
			String searchPackageCode, String UpdateAndClearCustomPackage,
			String runtest) {
		if (runtest.equalsIgnoreCase("true")) {
			...
			// Set Block Start and End dates
			TestReporter.logStep("Set Block Start and End dates");
			groupProfilePage
					.setBookingSourceAndBlockManagement(setBlockDatesAndSelectResort);
			...
			TestReporter.assertTrue(groupProfilePage.pageLoaded(),
					"Verify GroupProfile page is loaded");
			TestReporter
					.logStep("Click on Edit Group button in BlockList table");
			grpPageContentFrame.clickEditBlock();
			...
			groupProfilePage.seletFilterResort(setBlockDatesAndSelectResort);
			...
			groupProfilePage.searchPackageCode(searchPackageCode);
			groupProfilePage.clickCustomizeLink(searchPackageCode);

			// Update and Clear Custom package.
			TestReporter.logStep("Update and Clear Custom package.");
			groupProfilePage
					.updateAndClearCustomPackage(UpdateAndClearCustomPackage);
			...
			// Save BlockInventory details.
			TestReporter
					.logStep("Navigate to GroupProfile - Display Block/Block Inventory page.");
			TestReporter
					.assertTrue(groupProfilePage.pageLoaded(),
							"Verify GroupProfile - Display Block/Block Inventory page is loaded");
			TestReporter.logStep("Save BlockInventory details");
			groupProfilePage.clickbtnSave();
