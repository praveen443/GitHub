...
import apps.alc.existingReservation.ExistingReservationCancelConfirmPage;
 ...
 * 
 * @author Venkatesh A
 * @version 1/07/2016 Venkatesh A - Original
 */

@Listeners({ core.Screenshot.class })
public class ALC_CreateViewCancelChildActivity {
	 ...
	 */
	@Test(dataProvider = "dataProvider", description = "DLR Guest Facing Role, Children Activities, Create, View, Cancel", groups = {
			"regression", "alc", "Search", "Discovery", "Offers",
			"Venue Information", "CC Guarantee", "Cancel",
			"Existing Reservation-Reservation tab" })
	public void creatViewCancelChildActivity(String testScenario, String role,
			String roleAndLocation, String searchGuest, String discover,
			String payment, String venuInfo, String reasonForCancel){
		/*
		...
		TestReporter.log("DefaultOfferOffer : " + defaultOfferValue);
		...
		TestReporter
				.logStep("Entering the Venue and Clicks on Book from shopping cart tab page");
		TestReporter.assertTrue(shoppingCartPage.pageLoaded(),
				"ShoppingCart Tab page failed to load");
		TestReporter.log("Selected Offer Title : "
				+ shoppingCartPage.getSelectedOfferDetails());
		shoppingCartPage.bookReservation(venuInfo);
		...
		ExistingReservationReservationTab_HistoryTab exiResReservationTab_HistoryTab = new ExistingReservationReservationTab_HistoryTab(driver);
		TestReporter
				.assertTrue(exiResReservationTab_TabNavigation.pageLoaded(),
		...
		String GuestName = paymentHistoryTab.getGuestName();
		summary.clickDone();
		...
		TestReporter.logStep("Click Cancel button");
		TestReporter.assertTrue(exResGSP.verifyExistingGuestResults(),
				"Expected Guest result not available");
		exResGSP.clickCancel();

		TestReporter.logStep("Verifying Reservation Details - Cancel Page");
		TestReporter
				.assertTrue(exiResReservationTab_DetailsTab.pageLoaded(),
						"Existing Reservation - Reservation - Details Tab page failed to load");
		exiResReservationTab_DetailsTab.validateReservationDetails(
		...
		TestReporter.logStep("Cancel Existing Reservation - Cancel Page");
		exiResReservationTab_DetailsTab.cancelExistingReservationDetailsTab(
				GuestName, reasonForCancel);

		TestReporter.logStep("Capture Cancelled Confirm Details");
		ExistingReservationCancelConfirmPage cancelConfirmPage = new ExistingReservationCancelConfirmPage(
				driver);
		TestReporter.assertTrue(cancelConfirmPage.pageLoaded(),
				"Cancelled Confirm - Details Tab page failed to load");
		cancelConfirmPage.getCancelConformationDetails();
		cancelConfirmPage.clickContinue();
		summary.clickDone();
