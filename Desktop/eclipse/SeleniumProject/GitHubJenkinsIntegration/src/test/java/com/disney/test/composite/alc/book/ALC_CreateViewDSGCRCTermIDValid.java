 ...
 * 
 * @author Venkatesh A
 * @version 01/20/2016 Venkatesh A - Original
 */

@Listeners({ core.Screenshot.class })
public class ALC_CreateViewDSGCRCTermIDValid {
	...
	public Object[][] scenarios() {
		try {
			...
			return dt.getTestScenariosByApp(application,
					"ALC_CreateViewDSGCRCTermIDValid");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	@Test(dataProvider = "dataProvider", description = "DLR Guest Facing Role, DinnerShows, TermID Validationt", groups = {
			"regression", "alc", "Search", "Discovery", "GC Payment",
			"CC Payment", "Payment History", "Payment History tab", "Offers" })
	public void createViewDSGCRCTermIDValid(String testScenario, String role,
			...
			String book, String payment, String giftCard) {
		/*
		...
		TestReporter
				.logStep("Navigating to Payment page and Do Gift Card Payment");
		paymentPage.capturePaymentDetails();
		paymentPage.addPayment();
		paymentPage.takePayment(giftCard);

		TestReporter
				.logStep("Payment Or Balance Details After Gift Card Payment");
		paymentPage.getPaymentOrBalanceDetails();
		...
		TestReporter.assertTrue(
				exiResReservationTab_TabNavigation.pageLoaded(),
				"Existing Reservation - Reservation - Details Tab page loaded");
		...
		TestReporter.assertTrue(
				exiResReservationTab_TabNavigation.pageLoaded(),
				"Existing Reservation - Reservation - History Tab page loaded");
