 ...
 * Guest Search with the invalid search data
 * 
 * @author Marella Satish
 * @version 12/28/2015 Marella Satish - original
 */
@Listeners({ core.Screenshot.class })
public class ALC_InvalidSearchGuest {
	// *******************
	// Test Class Fields
	// *******************
	...
	public Object[][] scenarios() {
		try {
			...
			return dt.getTestScenariosByApp(application, "InvalidSearchGuest");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 */
	@Test(dataProvider = "dataProvider", description = "Test for Invalid Guest Search With NGF.DLR.A.Admin Role, US Guest", groups = {
			"regression", "alc", "Error Search (NEG)",
			"New Reservation-Guest Search tab" })
	public void InvalidSearchGuest(String testScenario, String role,
			String roleAndLocation, String guestSearch, String runTest){
		if (runTest.equalsIgnoreCase("true")) {
			...
			TestReporter
					.logStep("Search for Guest with Invalid guest information");
