...
import apps.dreams.GroupSearchWindow.AddNewSalesChannel;
...
public class Dreams_Block_Booking_Package {
	...
	public Object[][] scenarios() {
		try {
			...
			return dt.getTestScenariosByApp(application, "BlockBookingPackage");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 */
	@Test(dataProvider = "blockScenario", description = "Block Booking Package", groups = {
			...
			"AddNewSalesChannel", "setBlockInventory",
			"SearchPackageCodeAndUpdate" })
	public void blockBookingPackage(String testScenario, String role,
			...
			String AddNewSalesChannel, String setBlockInventory,
			String SearchPackageCodeAndUpdate, String runtest) {
		if (runtest.equalsIgnoreCase("TRUE")) {
			...
			grpPageContentFrame.guestSearchNameOnly(contactGuestSearch);
			...
			// Click on New in GroupProfile - Display Block/Block Inventory
			// page.
			TestReporter
			...
			TestReporter.logStep("Navigate to Add New Sales Channel Window.");
			AddNewSalesChannel addNewSalesWindow = new AddNewSalesChannel(
					driver);
			TestReporter.logStep("Enter New Sales channel Info and add them.");
			addNewSalesWindow.addNewSalesChannel(AddNewSalesChannel);
			...
			/*
			 * //Click on Show/HideBlocksList button.
			 * TestReporter.logStep("Click on Show/HideBlocksList button.");
			 * groupProfilePage.clickbtnShowHideBlocksList();
			 * 
			 * //Click on Edit Group button in Show/Hide BlocksList table.
			 * TestReporter
			 * .logStep("Click on Edit Group button in Show/Hide BlocksList");
			 * groupProfilePage.clickEditGroup();
			 */

			// Click on button Go-To-BlockInventory.
			TestReporter
					.logStep("Navigate to GroupProfile - Display Block/Block Inventory page.");
			TestReporter
					.assertTrue(groupProfilePage.pageLoaded(),
			...
			TestReporter.logStep("Click on link Packages");
			groupProfilePage.clickPackages();

			// Search Package Code and click on Customize link
			TestReporter.logStep("Navigate to Group Packages window.");
			TestReporter.assertTrue(groupProfilePage.isPageLoaded(),
					"Verify Groups - Package window is loaded");
			TestReporter
					.logStep("Search Package Code and Click on Customize link");
			groupProfilePage.searchPackageCode(SearchPackageCodeAndUpdate);
			groupProfilePage.clickCustomizeLink(SearchPackageCodeAndUpdate);

			// Update and Clear Custom package.
			TestReporter.logStep("Update and Clear Custom package.");
			groupProfilePage
					.updateAndClearCustomPackage(SearchPackageCodeAndUpdate);
			groupProfilePage.clickConfirmAllInCustomPkg();
			...
			// Verifying whether the searched package info exists in the table.
			TestReporter.logStep("Navigate to Group Packages window.");
			TestReporter.assertTrue(groupProfilePage.isPageLoaded(),
					"Verify Groups - Package window is loaded");
			TestReporter
					.logStep("Verifying whether the searched package info exists in the table.");
			groupProfilePage
					.verifySearchedPackageDetails(SearchPackageCodeAndUpdate);
