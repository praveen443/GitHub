 ...
 *           Creating a new table Service and Modify, Search and Cancel the
 *           Reservation.
 * @Precondition: N/A
...
public class ALC_BookModifyCancel {
	// *******************
	// Test Class Fields
	// *******************
	...
	public Object[][] scenarios() {
		try {
			...
			return dt.getTestScenariosByApp(application, "BookModifyAndCancel");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 */
	@Test(dataProvider = "dataScenarios", description = "Create a table Service and modify, Search and Cancel the Resevation.", groups = {
			"regression", "alc", "Quick Book", "Discovery", "Offers", "Search",
			"CC Guarantee", "Modify", "Cancel",
			"Existing Reservation-Reservation tab" })
	public void BookModifyCancel(String testScenario, String role,
			String roleAndLocation, String SelectAndEnterTableServiceInfo,
			String GuestSearchInfo, String setGuestDetailsCellPhone,
			String SetAllTermsAndBookResv, String makeCreditCardPayment,
			String ExistingGuestSearch, String ModifyTableServiceDetails,
		...
		Sleeper.sleep(3000);
		new WindowHandler().swapToWindow(driver, application);
		...
		// Enter the Cellphone number in Guest details.
		...
		TestReporter.logStep("Enter the Cellphone number in Guest details.");
		newEditGuestPage.enterGuestInformation(setGuestDetailsCellPhone);

		// Click on button Discover.
		TestReporter.logStep("Click on Discover button.");
		// NewEditGuest newEditGuestPage = new NewEditGuest(driver);
		newEditGuestPage.clickDiscoverButton();
		...
		TestReporter
				.logStep("Set Accept All-terms and conditions and Click on Book.");
		shoppingCartPage.bookReservation(SetAllTermsAndBookResv);
		...
		paymentPage.applySettlement(makeCreditCardPayment);

		// Capture Reservation details and click on Marketing Preferences
		// Required.
		...
		// Capturing the reservation number
		capturedResvNumber = summaryPage.getTheReservationNumber();
		TestReporter.logStep("Reservation number is : " + capturedResvNumber);

		// Click on Marketing Preferences Required.
		TestReporter.logStep("Click on Marketing Preferences Required.");
		summaryPage.clickMarketingPreferences();

		// Verify PTOI Page is displayed And click Unable-To-confirm.
		TestReporter
				.logStep("Verify whetherPTOI Page is displayed And click Unable-To-confirm.");
		summaryPage.verifyPTOIPageAndclickUnableToconfirm();
		...
		// Click Edit in Reservation details.
		TestReporter.logStep("Click Edit in Reservation details");
		ExisResv_ResvTab_DetailsTabpage.clickEditInResvDetails();

		// Navigate to the Discover page in ExistingReservationTab.
		// Enter Discovery Criteria for modifying the Reservation.
		TestReporter
				.logStep("Navigating to the Discover page in ExistingReservationTab.");
		ExistingReservationDiscoverPage existngResvDiscoverPage = new ExistingReservationDiscoverPage(
				driver);
		TestReporter.assertTrue(existngResvDiscoverPage.pageLoaded(),
				"Existng Reservation DiscoverPage did not load");
		TestReporter
				.logStep("Enter Discovery Criteria for modifying the Reservation");
		existngResvDiscoverPage
				.enterDiscoverCriteria(ModifyTableServiceDetails);

		// Navigating to the Offer page in ExistingReservationTab.
		// Click on Go To Shopping Cart button.
		TestReporter
				.logStep("Navigating to the Offer page in ExistingReservationTab.");
		ExistingReservationOffersPage existngResvOfferPage = new ExistingReservationOffersPage(
				driver);
		TestReporter.assertTrue(existngResvOfferPage.pageLoaded(),
				"Existng Reservation OfferPage did not load");
		TestReporter.logStep("Click on Go To Shopping Cart button");
		existngResvOfferPage.goToShoppingCart();

		// Navigate to the ExistingResvShoppingCart Page.
		// Accept and Confirm the Modified Booking.
		TestReporter
				.logStep("Navigating to the ExistingResv ShoppingCart Page.");
		ExistingReservationShoppingCartPage existResvShoppingCartPage = new ExistingReservationShoppingCartPage(
				driver);
		TestReporter.assertTrue(existResvShoppingCartPage.pageLoaded(),
				"Existing Reservation ShoppingCart page failed to load");
		TestReporter
				.logStep("Set Accept-All-terms and click on Modify button.");
		existResvShoppingCartPage.acceptAllTermsAndModify();

		// Navigate to the ExistingResv Confirm Page.
		// Accept and Confirm the Modified Booking.
		TestReporter.logStep("Navigating to the ExistingResv Confirm Page.");
		ExistingReservationConfirmPage existConfirm = new ExistingReservationConfirmPage(
				driver);
		TestReporter.assertTrue(existConfirm.pageLoaded(),
				"Existing Reservation Confirm failed to load");
		TestReporter.logStep("Accept and Confirm the Modified Booking");
		existConfirm.acceptAndConfirmNewOrModifiedBooking();
