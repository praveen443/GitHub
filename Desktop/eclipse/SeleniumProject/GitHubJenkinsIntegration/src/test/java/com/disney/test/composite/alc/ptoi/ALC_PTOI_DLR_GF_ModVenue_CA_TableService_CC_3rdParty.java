...
public class ALC_PTOI_DLR_GF_ModVenue_CA_TableService_CC_3rdParty {
	// *******************
	// Test Class Fields
	// *******************
	...
	public Object[][] scenarios() {
		try {
			...
			return dt.getTestScenariosByApp(application,
					"DLR_GF_ModVenue_CA_TableService_CC_3rdParty");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	@Test(dataProvider = "dataProvider", description = "Test PTOI Marketing With DLR Guest Facing Role, California Guest, Table Service Event, CC Payment, 3rd Party", groups = {
			...
			"CC Guarantee", "PTOI", "PTOI window", "Modify", "Database" })
	public void book(String testScenario, String role, String roleAndLocation,
			...
			String summaryScenario, String modDiscovery,
			String modSummaryScenario) {
		/*
		...
		TestReporter
				.logStep("Search for the Existing Reservation and Proceed to Modify");
		ExistingReservationGuestSearchPage guestSerch = new ExistingReservationGuestSearchPage(
				driver);
		TestReporter.assertTrue(guestSerch.pageLoaded(),
				"Existing Reservation Guest Search failed to load");
		guestSerch.searchByReservationToModify(reservationNumber);

		TestReporter.logStep("Continue Moving To Modify");
		ExistingReservationReservationTab_DetailsTab details = new ExistingReservationReservationTab_DetailsTab(
				driver);
		TestReporter.assertTrue(details.pageLoaded(),
				"Existing Reservation Reservation Details failed to load");
		TestReporter.assertTrue(
				details.getReservationDetailsHeaderInformation().contains(
						reservationNumber), "The reservation details ["
						+ details.getReservationDetailsHeaderInformation()
								.trim().replace(" ", "")
						+ "] did not contain the reservation number ["
						+ reservationNumber.trim() + "].");
		details.clickEdit();

		TestReporter
				.logStep("Enter Discovery Criteria for modifying the Reservation");
		ExistingReservationDiscoverPage existResDiscover = new ExistingReservationDiscoverPage(
				driver);
		TestReporter.assertTrue(existResDiscover.pageLoaded(),
				"Existing Reservation Discover failed to load");
		existResDiscover.enterDiscoverCriteria(modDiscovery);

		TestReporter.logStep("Select a Modified Offer");
		ExistingReservationOffersPage existOffers = new ExistingReservationOffersPage(
				driver);
		TestReporter.assertTrue(existOffers.pageLoaded(),
				"Existing Reservation Offer failed to load");
		existOffers.goToShoppingCart();

		TestReporter.logStep("Process Modified Shopping Cart");
		ExistingReservationShoppingCartPage existHoppingCart = new ExistingReservationShoppingCartPage(
				driver);
		TestReporter.assertTrue(existHoppingCart.pageLoaded(),
				"Existing Reservation Offer failed to load");
		existHoppingCart.acceptTermsAndBook();
		...
		existConfirm.acceptAndConfirmNewOffer();

		TestReporter.logStep("Apply Settlement and Continue");
		ExistingReservationPayment existPayment = new ExistingReservationPayment(
				driver);
		TestReporter.assertTrue(existPayment.pageLoaded(),
				"Existing Reservation Payment failed to load");
		existPayment.applySettlementAndContinue(paySettle);

		TestReporter.logStep("Summary Page");
		ExistingReservationSummaryPage existSummary = new ExistingReservationSummaryPage(
				driver);
		TestReporter.assertTrue(existSummary.pageLoaded(),
				"The Summary Tab failed to load");
		TestReporter.log("Reservation Number: " + reservationNumber);
		reservationNumber = existSummary.getReservationNumber();
		summary.handlePtoi(modSummaryScenario, primaryName);

		verifyPtoiInDatabases(modSummaryScenario);
	}
