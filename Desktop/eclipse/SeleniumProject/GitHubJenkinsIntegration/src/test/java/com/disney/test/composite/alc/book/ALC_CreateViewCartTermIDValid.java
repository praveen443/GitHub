...
public class ALC_CreateViewCartTermIDValid {
	// *******************
	// Test Class Fields
	// *******************
	 ...
	 */
	@Test(dataProvider = "dataProvider", description = "Test for CreateViewCartTermID Validation With GF.Admin.IT.Administrator Role", groups = {
			"regression", "alc", "Search", "Payment History tab", "Discovery",
			"Offers", "Venue Information", "CC Guarantee", "Payment History" })
	public void CreateViewCartTerm(String testScenario, String role,
			String roleAndLocation, String searchGuest, String EditGuest,
			String CreateViewCart, String BookReservation, String PaySettle){
		...
		// driver to be used by the test
		// The last argument in .initialize() force the use of the 32bit IE
		// webdriver
		...
		// Log a simple description of each step
		// Instantiate the necessary page class
		// / Assert that the page was loaded
		// Interact with the classes methods
		...
		TestReporter
				.logStep("Entering NewEditGuest << No Edit Changes in Test Case >>");
		...
		TestReporter.assertTrue(discover.pageLoaded(),
				"The Discover page did not load.");
		discover.enterDiscoverCriteria(CreateViewCart);
		...
		TestReporter.logStep("Entering the Venue Information and Booking");
		cart.bookReservation(BookReservation);

		TestReporter.logStep("Clicking Set Guarantee to Apply Settlement");
		Payment payment = new Payment(driver);
		TestReporter.assertTrue(payment.pageLoaded(),
				"Payment page failed to load");
		payment.applySettlement(PaySettle);

		TestReporter.logStep("Capture Summary Details");
		Summary summary = new Summary(driver);
		TestReporter.assertTrue(summary.pageLoaded(),
				"Summary page failed to load");
		summary.captureSummaryDetails();
		...
		TestReporter.logStep("View Reservation");
		ergs.clickView();

		ExistingReservationReservationTab_DetailsTab erdt = new ExistingReservationReservationTab_DetailsTab(
				driver);
		erdt.pageLoaded();

		TestReporter.logStep("Showing Details on Details Tab");
		erdt.getDetails();
	}
}
